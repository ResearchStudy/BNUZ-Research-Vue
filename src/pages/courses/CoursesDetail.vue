<template>
<div style="width: 80%;margin-left: 10%;margin-top: 50px;display: inline-flex;justify-content: space-between">
  <div style="width: 80%">
    <div style="border: 1px dashed #f2f2f2;padding: 20px;display: flex">
      ...
      <div style="width: 100%;padding-top: 45px">
        ...
        <div class="info">
          适合人群: {{info.suitable_for_crowd}}
        </div>
        <div  class="info">
          研学城市: {{info.address.city_name ? info.address.city_name : ''}}
        </div>
        <div  class="info">
          行程天数: {{info.travel_days }}天
        </div>
        <div  class="info">
          研学日期: {{info.startTime}}月 - {{info.endTime}}月
        </div>
        <div  class="info">
  ...
  <div style="width: 25%;border: 1px dotted #f2f2f2;margin-left: 5%">
    ...
    <div v-for="course in similarCourses" :key="course.id"  @click="navigateToCoursesById(course.id)">
