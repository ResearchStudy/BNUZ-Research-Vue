    ...
    <div style="width: 78%;margin-left: 11%">
          ...
          <div style="padding: 10px;display: flex;justify-content: space-between">
            <div>
              <span
                ...
                :style="activeTab === 'course' ? {color: '#409EFF'} : ''"
                style="cursor: pointer"
              >最新课程</span> |
              <span
                @mouseenter="activeTab = 'information'"
                :style="activeTab === 'information' ? {color: '#409EFF'} : ''"
                style="cursor: pointer"
              >最新资讯</span>
            </div>
            <span class="more" @click="moreClick">
              <i class="el-icon-circle-plus-outline"></i>MORE
            </span>
          </div>
          <div>
            <div
              style="display: flex;height: 32px;line-height: 32px;overflow: hidden;padding: 2px 10px"
        ...
        <div style="width: 70%;margin-left: 2%">
          <el-carousel height="329px" style="width: 100%">
            <el-carousel-item
              ...
              :key="item.id"
              @click.native="navigateToCourseDetail(item.id)"
              style="cursor: pointer"
            >
      ...
      <div>
        <div style="display: flex;justify-content: space-between">
          <h2>热门课程 |</h2>
          <span class="more" @click="$router.push({path: '/courses/'})">
            <i class="el-icon-circle-plus-outline"></i>MORE
          </span>
        </div>
        <div style="display: flex;">
          <el-card
            ...
            v-for="(course,index) in coursesList.slice(0,4)"
            shadow="hover"
            :key="course.id"
            :style="{width: '60%', marginLeft: index !== 0 ? '40px' : '0px'}"
            @click.native="$router.push({path: '/courses/' + course.id})"
          >
            ...
            <div style="padding: 12px;">
              ...
              >{{course.title.length > 12 ? course.title.substring(0, 12) + '...' : course.title}}</el-link>
              <div
                style="color: #9d9d9d;height: 51px;overflow: hidden;font-size: 14px;text-overflow:ellipsis"
              >
        ...
        <div style="display: flex;margin-top: 30px">
          <el-card
            :body-style="{ padding: '0px' }"
            v-for="(course,index) in coursesList.slice(4,8)"
            shadow="hover"
            :key="course.id"
            :style="{width: '60%', marginLeft: index !== 0 ? '40px' : '0px'}"
            @click.native="$router.push({path: '/courses/' + course.id})"
          >
            ...
            <div style="padding: 14px;">
              ...
              >{{course.title.length > 12 ? course.title.substring(0, 12) + '...' : course.title}}</el-link>
              <div
                style="color: #9d9d9d;height: 51px;overflow: hidden;font-size: 14px;overflow: hidden;font-size: 14px;text-overflow:ellipsis"
              >
                <p>{{course.description.length > 32 ? course.description.substring(0, 32) + '...' : course.description}}</p>
              </div>
            </div>
      ...
      <div class="hot-city-group" ref="hotCityGroup">
        ...
        <div @mouseenter="showArrow = true" @mouseleave="showArrow = false">
          <div class="left-arrow" v-show="showArrow" @click="scroll(true)">
            <img src="../../assets/img/arrow-left.png" alt />
          </div>
          <div class="right-arrow" v-show="showArrow" @click="scroll(false)">
